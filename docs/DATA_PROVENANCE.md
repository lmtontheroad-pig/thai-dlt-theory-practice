...
当前保留329道稳定ID原始记录，但SafeDrive没有公布后台随机题池总量，因此不能声称百分之百覆盖。复核发现 `SDLT-000160` 与 `SDLT-000026` 是同一组交通手势图片题的跨版本重复记录：图片编号集合相同，选项字母排列不同，正确图均为编号32。原始记录不删除，网页练习通过 `source/excluded_questions.json` 排除 `SDLT-000160`，实际提供328道题。
